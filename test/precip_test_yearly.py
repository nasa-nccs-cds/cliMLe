...
from cliMLe.layers import Layer
...
layers = [ Layer( "solu", pcDataset.getInputDimension(), batch_size=batchSize ), Layer( "solu", trainingDataset.getOutputSize() ) ]
...
def learning_model_factory( weights = None ):
    return LearningModel( inputDataset, trainingDataset, layers, batch=batchSize, lrate=learnRate, loss_function=loss_function, momentum=momentum, decay=decay, nesterov=nesterov, orthoWts=orthoWts, epocs=nEpocs, vf=validation_fraction, weights=weights )
